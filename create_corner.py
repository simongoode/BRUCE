...
N_LIVE = 10  # Number of live points (calibrated to match Bilby compression)
N_DELETE = int(N_LIVE * 0.7)  # Number of points to delete per iteration
...
# 1. Extract samples as a DataFrame for desired parameters
# 'samples' is NestedSamples object; get the posterior samples
try:
    data_to_plot = samples.posterior_samples[parameters_to_plot]
except AttributeError:
    # fallback in case .posterior_samples is not available; try direct access
    import pandas as pd
    data_to_plot = pd.DataFrame({k: samples[k] for k in parameters_to_plot})

# 2. Ordered label list
ordered_labels = [labels[param] for param in parameters_to_plot]

# 3. Truth values in correct order
ordered_truths = [injection_params[param] for param in parameters_to_plot]

# --- Generate the Corner Plot ---
num_samples = len(data_to_plot)
output_filename = f'/home/sgoode/BRUCE/results/bruce_corner.png'
